...
from ...db.sql.delegado.sql_delegado import SQL_INSERT_DELEGADO
from ...db.sql.delegado.sql_delegado_select import SQL_SELECT_DELEGADO
...
class Delegado():
     async def create_delegado(self, user_json_create: DelegadoCreate, conn: AsyncConnectionPool):
          pessoa_id = 0
          try:
               async with conn.transaction():
                    async with conn.cursor() as cursor:
                         ...
                         if pessoa_id:
                              ...
                              await cursor.execute(SQL_INSERT_DELEGADO(user_json_create.delegado, pessoa_id))
     ...
     async def get_delegado(self, cpf:str, conn: AsyncConnectionPool): 
          try:
               async with conn.cursor(row_factory=dict_row) as cursor:
               
                    await cursor.execute(SQL_SELECT_DELEGADO(cpf))
     ...
     async def update_delegado(self, user_json_update, conn: AsyncConnectionPool): 
               try:
